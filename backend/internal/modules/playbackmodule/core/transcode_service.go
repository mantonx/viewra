...
func (ts *TranscodeService) GetDashboardData() (*TranscodingDashboardData, error) {
	...
	for _, session := range activeSessions {
		...
		if session.Progress != nil {
			view.Progress = session.Progress.PercentComplete
		}

		if session.Request != nil {
			view.InputPath = session.Request.InputPath
			view.Quality = session.Request.Quality
			view.SpeedPriority = session.Request.SpeedPriority
		}
