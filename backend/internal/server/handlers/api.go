...
func ApiRootHandler(c *gin.Context) {
	...
	for _, route := range registeredRoutes {
		if route.Path == "/api/v1/users" {
			endpointsMap["users"] = route.Path
		}
		if route.Path == "/api/v1/media" {
			endpointsMap["media"] = route.Path
		}
		if route.Path == "/api/v1/plugins" {
			endpointsMap["plugins"] = route.Path
		}
		if route.Path == "/swagger/index.html" {
			endpointsMap["docs"] = route.Path
		}
		if route.Path == "/api" {
			endpointsMap["self"] = route.Path
		}
	}
