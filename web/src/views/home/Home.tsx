...
export const Home = () => {
  ...
  const libraries =
    librariesData?.status === 200 && 'libraries' in librariesData.data
...
interface HeroSectionProps {
  ...
  tvCount: number
  musicCount: number
  searchHeroData: ReturnType<typeof useSearchHeroData>['data']
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="relative p-8 pb-10">
        ...
        <p className="text-lg text-neutral-600 dark:text-neutral-400 leading-relaxed max-w-2xl mb-8">
          Your self-hosted media server for movies, TV shows, and music.
        </p>

        {/* Search Hero */}
        {!isLoadingSearch && searchHeroData && (
          <SearchHero data={searchHeroData} className="mb-10" />
        )}

        {/* Stats cards */}
        <div className="grid grid-cols-3 gap-4 max-w-xl">
          <StatCard label="Movies" count={movieCount} />
          <StatCard label="TV Shows" count={tvCount} />
          <StatCard label="Albums" count={musicCount} />
        </div>
      </div>
    </div>
  )
