...
import { Card, CardHeader, CardContent, Button, Modal, Loading } from '@/components/ui'
    ...
    <div className="h-full overflow-auto">
      <div className="p-8 page-enter">
        ...
        <div className="mt-6 max-w-4xl">
          <Card>
            <CardHeader className="border-b border-neutral-100 dark:border-neutral-800">
              <h2 className={cn('text-lg font-semibold', text.primary)}>Users</h2>
            </CardHeader>
            <CardContent className="p-0">
              {usersLoading ? (
                ...
                <div className="p-8 text-center text-red-600 dark:text-red-400">{usersError}</div>
              ) : users.length === 0 ? (
                <div className={cn('p-8 text-center', text.secondary)}>No users found</div>
              ) : (
                <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
                  {users.map((user) => (
                    <div
                      key={user.id}
                      className="flex items-center justify-between px-6 py-4 hover:bg-neutral-50 dark:hover:bg-neutral-900/50"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className={cn('text-sm', text.secondary)}>@{user.username}</span>
                        </div>
                      </div>
        ...
        <Modal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} title="Create User">
          <CreateUserForm onSuccess={handleCreateSuccess} onCancel={() => setShowCreateModal(false)} />
        </Modal>
