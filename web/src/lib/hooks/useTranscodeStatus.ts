...
import type { InternalApiHandlersTranscodeJobResponse as TranscodeJobResponse } from '../api/generated/models'
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (response.status !== 200) {
        throw new Error('error' in response.data ? String(response.data.error) : 'No transcode job found')
      }
      return response.data as TranscodeJobResponse
    },
...
export const isTranscodeComplete = (status: TranscodeJobResponse | undefined): boolean => {
  return status?.status === 'completed'
}

// Helper to check if transcode failed
export const isTranscodeFailed = (status: TranscodeJobResponse | undefined): boolean => {
  return status?.status === 'failed'
}

// Helper to check if transcode is in progress
export const isTranscodeInProgress = (status: TranscodeJobResponse | undefined): boolean => {
...
export const getTranscodeMessage = (status: TranscodeJobResponse | undefined): string => {
  if (!status) {
