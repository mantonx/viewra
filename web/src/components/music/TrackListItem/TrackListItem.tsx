    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          {track.artist && (
            <p className={cn('text-xs truncate', text.secondary)}>{track.artist}</p>
          )}
          {/* Metadata badges */}
          <div className="flex items-center gap-1.5">
            ...
            {track.bitrate && (
              <span className={cn('px-1.5 py-0.5 text-[10px] font-medium rounded', bg.tertiary, text.secondary)}>
                {track.bitrate} kbps
              </span>
            )}
          </div>
      ...
      {track.disc_number && track.disc_number > 1 && (
        <div className="shrink-0">
          <span className={cn('px-2 py-1 text-xs rounded', bg.tertiary, text.secondary)}>
      ...
      {/* Duration */}
      <div className={cn('text-sm shrink-0 w-12 text-right', text.tertiary)}>
