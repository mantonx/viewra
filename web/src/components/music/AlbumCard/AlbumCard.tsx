...
const AlbumCard = ({ album, onClick }: AlbumCardProps) => {
  return (
    <MediaCard
      ...
      infoContent={
        <>
          <h3 className="font-semibold text-sm line-clamp-2 mb-1 text-neutral-900 dark:text-neutral-50">{album.album ?? 'Unknown Album'}</h3>
          <p className="text-xs text-neutral-600 dark:text-neutral-400 mb-2 line-clamp-1">{album.artist ?? 'Unknown Artist'}</p>
