...
interface TranscoderSectionRendererProps {
...
const TranscoderSectionRenderer: React.FC<TranscoderSectionRendererProps> = ({
  data,
  isNerdMode,
  onToggleNerdMode,
  onSessionAction,
  ...
  // Helper function to extract a proper title from filename
  const getContentTitle = (filename: string) => {
    if (!filename) return 'Unknown Content';
    
    // Extract filename without path and extension
    const basename = filename.split('/').pop() || filename;
    const nameWithoutExt = basename.replace(/\.[^/.]+$/, '');
    
    // Clean up the name: replace underscores/dots with spaces, title case
    ...
    return cleanName || 'Unknown Content';
  };
                  ...
                  <div className="flex items-center space-x-3">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-medium text-white truncate max-w-md" title={session.input_filename}>
                        {getContentTitle(session.input_filename)}
                      </span>
                    </div>
                ...
                <div className="mb-3">
                  ...
                  <div className="w-full bg-slate-700 rounded-full h-2">
                    <div 
                      ...
                      style={{ width: `${Math.max(0, Math.min(100, session.progress * 100))}%` }}
                    />
                  </div>
                </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="text-sm space-y-1">
                          <div className="flex justify-between">
                            ...
                            <span className="text-white">{session.client_ip || 'Not available'}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-400">Device:</span>
                            <span className="text-white">{session.client_device || 'Not detected'}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-400">Started:</span>
                            <span className="text-white">
                              {session.start_time ? new Date(session.start_time).toLocaleTimeString() : 'Unknown'}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-400">ETA:</span>
                            <span className="text-white">
                              {session.estimated_time_left && session.estimated_time_left !== 'Unknown' 
                                ? session.estimated_time_left 
                                : 'Calculating...'}
                            </span>
                          </div>
                        </div>
...
export default TranscoderSectionRenderer; 