...
// TestVideoInfoCache_GetDeletedFile tests that cached entries persist even if file is deleted
// This is intentional - we skip os.Stat on Get for performance with network mounts.
// Cache expiration is time-based only.
func TestVideoInfoCache_GetDeletedFile(t *testing.T) {
	cache := &videoInfoCache{
		entries: make(map[string]*videoInfoCacheEntry),
	}
	...
	// Get should still return cached value (we don't stat on Get for performance)
	result := cache.Get(testFile)
	if result == nil {
		t.Error("cache.Get() for deleted file returned nil, want cached value (we skip stat for performance)")
	}
}

// TestVideoInfoCache_FileModificationDoesNotInvalidate tests that file modification
// does NOT invalidate cache (we skip stat for performance with network mounts).
// Cache is time-based only - entries expire after cacheMaxAge (24 hours).
func TestVideoInfoCache_FileModificationDoesNotInvalidate(t *testing.T) {
	cache := &videoInfoCache{
		entries: make(map[string]*videoInfoCacheEntry),
	}
	...
	// Cache should still return the cached value (we don't check mtime for performance)
	result := cache.Get(testFile)
	if result == nil {
		t.Error("cache.Get() after file modification returned nil, want cached value (mtime check skipped for performance)")
	}
}
...
// Set now works for any path since we removed the stat check for performance.
func TestVideoInfoCache_SetNonExistentFile(t *testing.T) {
	...
	if !exists {
		t.Error("cache.Set() did not store entry, expected entry to be cached")
	}
}
