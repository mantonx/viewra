...
// StartParams contains all parameters needed to start a transcode session.
// This struct is used to avoid long parameter lists and make the API cleaner.
...
func (s *TranscodeSession) Start(params StartParams) error {
	...
	s.SegmentDurationSec = params.Profile.SegmentDuration

	// Build FFmpeg arguments
	args := s.buildFFmpegArgs(params)
	...
	s.FFmpegCmd = createFFmpegCommand(s.ctx, args, params.Config, s.logger)
...
func (s *TranscodeSession) GetLogWriter() *logging.LogWriter {
	return s.logWriter
}
