...
func ExampleWithRetry_basic() {
	...
	err := transcoding.WithRetry(ctx, transcoding.DefaultRetryConfig(), func() error {
		attempts++
		if attempts < 3 {
			// Transient error - will retry
			return transcoding.NewRetryableError(errors.New("temporary failure"))
		}
...
func ExampleWithRetry_customConfig() {
	...
	config := transcoding.RetryConfig{
		MaxRetries:     5,
	...
	err := transcoding.WithRetry(ctx, config, func() error {
		// Your operation here
		return nil
	})
...
func ExampleWithRetry_contextCancellation() {
	...
	err := transcoding.WithRetry(ctx, config, func() error {
		return transcoding.NewRetryableError(errors.New("transient"))
	})
