...
func (m *mockConfigProvider) GetConfig(ctx context.Context) *TranscodeConfig {
	return m.config
}
...
func TestNewSessionManager(t *testing.T) {
	...
	manager := NewSessionManager(config, logger)
...
func TestNewSessionManager_NilConfig(t *testing.T) {
	...
	manager := NewSessionManager(nil, logger)
	...
	if manager.config.HardwareAccel == "" {
		t.Error("Hardware acceleration should be set to default")
	}
}
...
func TestSetLogStore(t *testing.T) {
	...
	tmpDir := t.TempDir()
	logStore, err := NewFFmpegLogStore(tmpDir, 48*time.Hour, logger)
	if err != nil {
...
func TestGetEffectiveConfig_WithProvider(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(os.Stdout, nil))
	baseConfig := DefaultTranscodeConfig()
	baseConfig.ToneMappingAlgorithm = "hable"

	manager := NewSessionManager(baseConfig, logger)

	// Without provider, should return base config
	effective := manager.getEffectiveConfig(context.Background())
	if effective.ToneMappingAlgorithm != "hable" {
		t.Errorf("Without provider, expected algorithm 'hable', got %q", effective.ToneMappingAlgorithm)
	}

	// With provider, should return provider's config
	providerConfig := DefaultTranscodeConfig()
	providerConfig.ToneMappingAlgorithm = "bt.2390"
