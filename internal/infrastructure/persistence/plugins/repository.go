...
func (r *Repository) ListPlugins(ctx context.Context) ([]appplugins.Plugin, error) {
	if common.IsPostgres(r.dbType) {
		...
		for i, row := range rows {
			result[i] = postgresPluginToApp(row)
		}
		return result, nil
	}
	...
	result := make([]appplugins.Plugin, len(rows))
	for i, row := range rows {
		result[i] = sqlitePluginToApp(row)
	}
	return result, nil
}
...
func (r *Repository) GetPlugin(ctx context.Context, id string) (appplugins.Plugin, error) {
	if common.IsPostgres(r.dbType) {
		...
		return postgresPluginToApp(row), nil
	}
	...
	return sqlitePluginToApp(row), nil
}
...
// sqlitePluginToApp converts a SQLite plugin row to the application type.
func sqlitePluginToApp(row sqlc_sqlite.Plugin) appplugins.Plugin {
	p := appplugins.Plugin{
		ID:         row.ID,
		Name:       row.Name,
		Version:    row.Version,
...
// postgresPluginToApp converts a PostgreSQL plugin row to the application type.
func postgresPluginToApp(row sqlc_postgres.Plugin) appplugins.Plugin {
	p := appplugins.Plugin{
		ID:          row.ID,
		Name:        row.Name,
		Version:     row.Version,
