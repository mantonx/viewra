...
// DeleteLibrary deletes a library and all its files comprehensively
func (lm *LibraryManager) DeleteLibrary(id uint) error {
	if !lm.initialized {
		return fmt.Errorf("library manager not initialized")
	}

	logger.Info("Starting comprehensive library deletion", "library_id", id)

	// Use the comprehensive deletion service
	deletionService := NewLibraryDeletionService(lm.db, lm.eventBus)
	// Note: Scanner manager would need to be set by the caller if available
	...
	if !result.Success {
		...
		return fmt.Errorf("failed to delete library: %s", result.Message)
	}
