...
func ProcessFileWithCheckpoint(
	ctx context.Context,
	deps *Deps,
	lib *library.Library,
	...
	fileInfo, err := scanutil.StatWithTimeout(ctx, checkpoint.FilePath, deps.Config.BaseFileTimeout)
	if err != nil {
	...
	case library.LibraryTypeMovies:
		mediaID, processErr = deps.MediaProcessor.ProcessMovie(ctx, lib.ID, &result, checkpoint, existingMediaCache)
	case library.LibraryTypeTV:
		mediaID, processErr = deps.MediaProcessor.ProcessTVEpisode(ctx, lib.ID, &result, checkpoint, existingMediaCache)
	case library.LibraryTypeMusic:
		mediaID, processErr = deps.MediaProcessor.ProcessMusicTrack(ctx, lib.ID, &result, checkpoint, existingMediaCache)
	default:
	...
	if err := deps.ScanRepos.ScanState.Upsert(ctx, scanState); err != nil {
		// Log error but don't fail the scan - scan state is for optimization, not critical
		deps.Logger.Warn("failed to update scan state",
			"file_path", checkpoint.FilePath,
			"error", err)
	}
...
func calculateProcessingTimeout(deps *Deps, fileSize int64) time.Duration {
	isRemote := deps.SystemProfile != nil && deps.SystemProfile.Storage.IsRemote
	return scanutil.CalculateProcessingTimeout(fileSize, scanutil.TimeoutConfig{
		BaseFileTimeout:      deps.Config.BaseFileTimeout,
		RemoteStorageTimeout: deps.Config.RemoteStorageTimeout,
		MaxExtraTimeout:      deps.Config.MaxExtraTimeout,
