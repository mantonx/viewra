...
// Tests for scan progress tracking, job completion, and discovery validation.
// Utility function tests (IsMediaFile, CalculateProcessingTimeout, StatWithTimeout)
// are in the scan/scanutil sub-package.
...
func TestValidateDiscovery(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			warnings := discovery.ValidateDiscovery(ctx, deps, 1, tt.filesDiscovered, tt.stats)
...
func TestCompleteSafely(t *testing.T) {
	t.Run("successful completion", func(t *testing.T) {
		...
		status.CompleteSafely(context.Background(), deps, job)
