...
CREATE INDEX IF NOT EXISTS idx_media_images_file_path
    ON media_images(file_path)
    WHERE file_path IS NOT NULL;

-- Unique constraint to prevent duplicate images
-- Allows for upserts based on the same combination
CREATE UNIQUE INDEX IF NOT EXISTS idx_media_images_unique
    ON media_images(media_type, entity_id, image_type, COALESCE(file_path, ''), COALESCE(file_hash, ''));
