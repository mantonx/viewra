...
// Provider returns the Anthropic provider implementation.
func (p *Plugin) Provider() *AnthropicProvider {
	return p.provider
}

// Initialize is called when the plugin is loaded.
func (p *Plugin) Initialize(ctx context.Context, req *pluginv1.InitRequest) (*pluginv1.InitResponse, error) {
	p.logger.Info("initializing Anthropic provider plugin",
...
// GetRoutes returns HTTP routes for Anthropic provider.
...
func (p *Plugin) Configure(ctx context.Context, req *pluginv1.Settings) (*pluginv1.ConfigureResponse, error) {
	...
	// Store chat model selection
	p.provider.SetChatModel(settings.ChatModel)
...
func (p *Plugin) HandleHTTP(ctx context.Context, req *pluginv1.PluginHTTPRequest) (*pluginv1.PluginHTTPResponse, error) {
	...
	// GET /health - Check Anthropic API connectivity
